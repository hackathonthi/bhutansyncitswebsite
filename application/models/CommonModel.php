...
class CommonModel extends CI_Model{
   ...
   //get user details
    ...
    function getchurnrateDetails($year=""){
        $query= $this->db->query("SELECT f.`Month`, f.`Churn_Rate` FROM `t_subscriber_bmobile_main` f WHERE f.`Year`='".$year."'")->
            result_array();
        return $query;
    }
    ...
    function getfixedlinedzongkhag($year=""){
         $query =$this->db->query( "SELECT f.`year`, f.`Month`,f.`Dzongkhag`,f.`total` FROM t_subscriber_fl_excel f WHERE f.`Year`='".$year."'")->
            result_array();
        return $query;
    }
